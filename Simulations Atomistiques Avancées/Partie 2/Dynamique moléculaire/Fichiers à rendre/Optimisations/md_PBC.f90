...
10  continue
  ...
  subroutine evolve(dt,rcut,box,pos,vel,for,epot, hbox)
    ...
    call forces(rcut,box,pos,for,epot, hbox)
    vel = vel + 0.5_DP * for / mass * dt
  ...
  subroutine forces(rcut,box,pos,for,epot, hbox)
    real(DP), intent(in)  :: box(:), hbox(:), rcut
...
program main
  ...
  call forces(rcut,box,pos,for,epot, hbox)
  do i = 1,nsteps
     call evolve(dt,rcut,box,pos,vel,for,epot, hbox)
