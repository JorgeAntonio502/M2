...
10  continue
  ...
  subroutine evolve(dt,rcut,box,pos,vel,for,epot,uc)
    ...
    call forces(rcut,box,pos,for,epot,uc)
    vel = vel + 0.5_DP * for / mass * dt
  ...
  subroutine forces(rcut,box,pos,for,epot,uc)
    real(DP), intent(in)  :: box(:), rcut, uc
...
program main
  ...
  call forces(rcut,box,pos,for,epot,uc)
  do i = 1,nsteps
     call evolve(dt,rcut,box,pos,vel,for,epot,uc)
