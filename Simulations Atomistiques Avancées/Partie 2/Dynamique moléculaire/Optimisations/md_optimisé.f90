...
10  continue
  ...
  subroutine evolve(dt,rcut,box,pos,vel,for,epot,hbox,uc)
    real(DP), intent(in)    :: dt, rcut, uc
    real(DP), intent(in)    :: box(:), hbox(:)
    ...
    do i = 1,size(pos,2) 
       call pbc(pos(:,i),box,hbox)
    end do
    call forces(rcut,box,pos,for,epot,hbox,uc)
    vel = vel + 0.5_DP * for / mass * dt
  ...
  subroutine potential(rcut,rsq,uc,u,w)
    real(DP), intent(in)  :: rsq, rcut, uc
  ...
  subroutine forces(rcut,box,pos,for,epot,hbox,uc)
    real(DP), intent(in)  :: box(:), hbox(:), rcut, uc
    ...
    do i = 1,size(pos,2)
       ...
       do j = i+1,size(pos,2)
          ...
          if (rijsq < rcut**2) then
             call potential(rcut,rijsq,uc,uij,wij) ! wij = -(du/dr)/r
             epot = epot + uij
             for(:,i) = for(:,i) + wij * rij
             for(:,j) = for(:,j) - wij * rij
          end if
...
program main
  ...
  call forces(rcut,box,pos,for,epot,hbox,uc)
  do i = 1,nsteps
     call evolve(dt,rcut,box,pos,vel,for,epot,hbox,uc)
