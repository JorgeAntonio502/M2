...
10  continue
  ...
  subroutine forces(rcut,box,pos,for,epot)
    ...
    integer               :: i, j
    real(DP)              :: ftmp(NDIM,size(pos,2),NTHREADS) ! stocke les forces partielles
    integer               :: RANK
    for = 0.0_DP
    ftmp = 0.0_DP
    epot = 0.0_DP
    ...
    ! rank du thread pour ftmp
       ...
       do j = i+1,size(pos,2)
          ...
          if (rijsq < rcut**2) then
             ...
             epot = epot + uij
             ftmp(:,i,RANK) = ftmp(:,i,RANK) + wij * rij
             ftmp(:,j,RANK) = ftmp(:,j,RANK) - wij * rij
             !for(:,i) = for(:,i) + wij * rij
             !for(:,j) = for(:,j) - wij * rij
          end if
       end do
