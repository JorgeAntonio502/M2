...
int main (int argc, char * argv[]) 
{
	...
	double const factor_norm = ( (L*L*L)/(4*M_PI*N_Si*N_O) )/nb_config;
	...
	{		
		// Boucle sur le premier type d'atomes
		for(i = num_it*N; i < num_it*N + N_O; i++)
		{
			// Boucle sur le second type d'atomes
			for(j = num_it*N + N_O; j < num_it*N + N; j++)
			{
