...
const int N = 100;  // Nombre de cycles de MC
float L = 40; // taille de la boîte de simulation (carrée)
double half_box = L/2;
...
float dl = L/9;//pow(2, (double) 1/6)*sigma; // distance entre les molecules du cristal
...
int main()
{
    ...
    double V = L*L;
    float norm = dl; // Longueur max des deplacements selon x et y
    ...
    for(int i = 0; i < N; i++) 
    {
    	...
    	for(int j = 0; j < N_particules; j++)
    	{
			...
			{
				P = P_tentative * exp( -(Ep_f-Ep_i) / K_B*T );
			}
