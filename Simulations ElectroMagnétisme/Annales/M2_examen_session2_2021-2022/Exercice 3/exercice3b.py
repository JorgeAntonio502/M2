...
#     Add second metal cylinder

diametre = 20          # diameter of cylinder: 6 cm
radius = diam/2.0     # radius of cylinder: 3 cm
i_center = 2*ie/3 - 2.5*lambda0  # i-coordinate of cylinder's center
j_center = je/2     # j-coordinate of cylinder's center
       
for i in range(1,ie):
  for j in range(1,je):
    dist2 = (i+0.5-i_center)**2 + (j-j_center)**2
    if dist2 <= radius**2: 
       caex[i,j] = ca[1]
       cbex[i,j] = cb[1]
    dist2 = (i-i_center)**2 + (j+0.5-j_center)**2
    if dist2 <= radius**2: 
       caey[i,j] = ca[1]
       cbey[i,j] = cb[1]
